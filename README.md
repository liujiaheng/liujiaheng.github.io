...
Internal page links, styles, and scripts share a content version to prevent stale pages during navigation. The campus photograph appears only in the homepage profile header, without blur and with a light overlay behind the text. Original image assets are preserved; display variants use lightweight web encodings. The portrait uses a transparent cutout derived from the original photograph, keeping its facial and clothing pixels; a short CSS fade blends the lower edge into the campus header.
